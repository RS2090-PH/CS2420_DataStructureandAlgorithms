...
def removeAll(item, lyst):
    """ Removes all instances of an item from a list. """
    if isEmpty(lyst) is True:
        return lyst
    elif first(lyst) == item:
        return rest(cons(first(lyst), removeAll(item, rest(lyst))))
    else:

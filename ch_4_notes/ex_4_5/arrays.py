...
Add the method __eq__ to the Array class. Python runs this method when an Array object appears as the left 
operand of the == operator. The method returns True if its argument is:
also an Array,
...
class Array(object):
    ...
    def __eq__(self, array_b):
        ret_value = None

        if type(array_b) != Array:
            return False
        if self.logicalSize != array_b.logicalSize:
            return False
        if self.logicalSize > 0:
            for count in range(self.logicalSize):
                if self.items[count] != array_b.items[count]:
                    return False
                else:
                    ret_value = True
            return ret_value
